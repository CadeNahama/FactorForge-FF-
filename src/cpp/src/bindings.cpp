...
PYBIND11_MODULE(quant_cpp, m) {
    ...
    // New: Expose Config struct
    py::class_<EnhancedRiskManager::Config>(m, "EnhancedRiskManagerConfig")
        .def(py::init<>())
        .def_readwrite("max_portfolio_risk", &EnhancedRiskManager::Config::max_portfolio_risk)
        .def_readwrite("max_position_size", &EnhancedRiskManager::Config::max_position_size);

    py::class_<EnhancedRiskManager, std::unique_ptr<EnhancedRiskManager>>(m, "EnhancedRiskManager")
        .def(py::init<const EnhancedRiskManager::Config&>())
        .def_static("Create", &EnhancedRiskManager::Create, py::arg("config"))
        ...
        .def("optimizePositionSizes", &EnhancedRiskManager::optimizePositionSizes)
        .def("applyRiskLimits", &EnhancedRiskManager::applyRiskLimits)
        .def("generateRiskReport", &EnhancedRiskManager::generateRiskReport)
        .def("clampSignals", &EnhancedRiskManager::clampSignals)
        .def("getMaxPortfolioRisk", &EnhancedRiskManager::getMaxPortfolioRisk)
        .def("getMaxPositionSize", &EnhancedRiskManager::getMaxPositionSize);
